...
class ProfileRepository {
  ...
  async getActiveListingsCount(userId) {
    try {
      ...
      logger.warn('Failed to get active listings count', { userId, error: error.message });
      return 0;
    }
  }
  ...
  async search(criteria) {
    try {
      ...
      if (criteria.query) {
        conditions.push(`(display_name ILIKE '%' || ${sql`${criteria.query}`} || '%' OR bio ILIKE '%' || ${sql`${criteria.query}`} || '%')`);
      }

      if (criteria.userType) {
        conditions.push(sql`user_type = ${criteria.userType}`);
      }

      if (criteria.location) {
        conditions.push(`location ILIKE '%' || ${sql`${criteria.location}`} || '%'`);
      }
