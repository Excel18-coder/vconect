 ...
 * Orchestrates between repository, validation, and external services
 */
  ...
  async createProduct(userId, productData, files) {
    try {
      ...
      if (!productData.title || !productData.description || !productData.price) {
        throw new ValidationError('Missing required fields: title, description, and price are required');
      }
      ...
      const productRecord = {
        ...
        max_order_quantity: productData.max_order_quantity ? parseInt(productData.max_order_quantity) : null,
        ...
        shipping_included: productData.shipping_included === 'true' || productData.shipping_included === true,
