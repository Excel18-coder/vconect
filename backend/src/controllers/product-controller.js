...
const createProduct = asyncHandler(async (req, res) => {
  ...
  if (userProfile.length === 0 || !allowedUserTypes.includes(userProfile[0].user_type)) {
    return sendError(res, 'Only sellers and landlords can create listings', null, 403);
  }
  ...
  return sendCreated(res, 'Product created successfully', {
    product: {
      ...product,
      final_price:
        parseFloat(product.price) * (1 - parseFloat(product.discount_percentage || 0) / 100),
    },
  });
});
  ...
  try {
    ...
    return sendSuccess(res, 'Products retrieved successfully', {
      products: products.map(product => ({
        ...
        final_price:
          parseFloat(product.price || 0) * (1 - parseFloat(product.discount_percentage || 0) / 100),
      })),
      pagination: {
        page: parseInt(page),
  ...
  try {
    ...
    if (category) {
      ...
      if (normalized === 'market') {
        const marketAliases = getCategoryAliases('market').map(s => s.toLowerCase());
        ...
        const aliases = getCategoryAliases(normalized).map(s => s.toLowerCase());
    ...
    if (search) {
      ...
      whereClauses.push(`(l.title ILIKE ${qParam} OR l.description ILIKE ${qParam})`);
    }
    ...
    if (min_price !== undefined && min_price !== null && String(min_price) !== '') {
    ...
    if (sortByKey) {
      ...
      const orderKey = String(order || 'DESC').toUpperCase() === 'ASC' ? 'ASC' : 'DESC';
